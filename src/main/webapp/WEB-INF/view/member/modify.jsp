...
<script type="text/javascript">
	$()
			.ready(
					function() {
						var passwordAfter = $(`<div class="error"> 비밀번호를 입력해주세요. <br/>  입력하지 않을 시 기존 비밀번호를 유지합니다.</div>`);
						var passwordConfirmAfter = $(`<div class="error"> 비밀번호를 확인해 주세요. <br/>  입력하지 않을 시 기존 비밀번호를 유지합니다.</div>`);
						...
						function errorDivAdd() {
							...
							var result = false;
							if ( nameEmpty || phoneEmpty || passwordEmpty || passwordConfirmEmpty) {
								$(".error").remove();
								if (nameEmpty) {
								...
								if ( $("#fpassword").val() != $("#passwordConfirm").val() ) {
									$("#passwordConfirmError").prepend(passwordConfirmAfter);
									result = true;
								}
							}
